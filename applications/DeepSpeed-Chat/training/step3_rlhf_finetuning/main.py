...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    train_data1= prompt_train_dataset.prompt_dataset[:1]
    eval = prompt_train_dataset.chosen_dataset[:1]
    print(f'train_data1:{train_data1}')
    print(f'eval:{eval}')
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            t1 = time.time()
            dt = t1 - t0
            reportStatistic("total-generate",dt)
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        t1 = time.time()
                        dt = t1 - t0
                        reportStatistic("total-train",dt)
